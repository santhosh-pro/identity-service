import { TenantFilter } from "src/infrastructure/database/tenant-filter";
import { FindConditions, FindOneOptions, Repository, SelectQueryBuilder, UpdateResult } from "typeorm";
...
export class BaseService<TRepository extends Repository<Entity>, Entity> implements IBaseService<Entity>{
  ...
  createQueryBuilder(alias = ''): SelectQueryBuilder<Entity> {
    return this.repository.createQueryBuilder(alias).where(alias + '.tenant_id=:tenantId', { tenantId: TenantFilter.tenantId });
  }
